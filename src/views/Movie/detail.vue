...
<script>
    ...
    export default {
        ...
        //通常可以通过$route.params.id获取到movieid
        //在组件中使用 $route 会使之与其对应路由形成高度耦合，从而使组件只能在某些特定的 URL 上使用，
        //限制了其灵活性。在路由文件中使用 props 将组件和路由解耦：这里就可以用下面的方式接收参数
        props:['movieId'],
        data(){
            return{
                detailMovie : {},
        ...
        mounted(){
            this.axios.get("/ajax/detailmovie?movieId="+this.movieId+"&optimus_uuid=D809CFB0FF1111EA8BC79F146444EC44CD717BBEE4AB4708A27735547C47B6C0&optimus_risk_level=71&optimus_code=10").then(res =>{
